...
import { z } from "zod";
import { getLatestTrip } from "@/actions/getLatestTrip";
  ...
  z.array(
    z.object({
      ...
      date: z
        ...
        .describe(
          'if the the itinerary item has a specified date - Like a concert or theatre performance, provide it here, if not use "flexible".'
        ),
      address: z
        .string()
...
export const generateItineraryIdeas = async () => {
  ...
  const chain = RunnableSequence.from([
    ChatPromptTemplate.fromTemplate(
      `You are an Itinerary creator. You must create 7 itinerary items that fits the user's budget, travel-style and pace from the following categories - events, restaurants, attractions, activities, hidden gems. Use the specified start date and end date to find events and activity within that time-span and they must be time-of-the-year appropriate. Your itinerary items must be based on the user input and you must take the specified budget, travel style, and pace into account! If mustSee is not equal to null, then you must create an item for every value in the mustSee field, if it is equal to null, then ignore the mustSee field. Follow these strict format instructions exactly: {format_instructions} User input: {entry}`
    ),
    llm,
    itineraryParser,
  ]);
